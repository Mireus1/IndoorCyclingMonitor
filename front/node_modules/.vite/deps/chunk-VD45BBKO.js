...
function useMenuButton(parameters = {}) {
  ...
  const menuContext = React4.useContext(DropdownContext);
  if (menuContext === null) {
...
function useOption(params) {
  ...
  const optionRef = React6.useRef(null);
  const selectOption = React6.useMemo(() => ({
    disabled,
    label,
    value,
    ref: optionRef,
...
function useOptionContextStabilizer(value) {
  const listContext = React7.useContext(ListContext);
  if (!listContext) {
  ...
  const localGetItemState = React7.useCallback((itemValue) => {
    if (itemValue !== value) {
  ...
  const localContextValue = React7.useMemo(() => ({
    dispatch,
...
function useSelect(props) {
  ...
  const optionValues = React8.useMemo(() => Array.from(options.keys()), [options]);
  const getOptionByValue = React8.useCallback((valueToGet) => {
  ...
  const isItemDisabled = React8.useCallback((valueToCheck) => {
    var _option$disabled;
    const option = getOptionByValue(valueToCheck);
    return (_option$disabled = option == null ? void 0 : option.disabled) != null ? _option$disabled : false;
  }, [getOptionByValue]);
  const stringifyOption = React8.useCallback((valueToCheck) => {
    const option = getOptionByValue(valueToCheck);
    if (!option) {
      return "";
    }
  ...
  const controlledState = React8.useMemo(() => ({
    selectedValues: value,
    open: openProp
  }), [value, openProp]);
  const getItemId = React8.useCallback((itemValue) => {
    var _options$get;
    return (_options$get = options.get(itemValue)) == null ? void 0 : _options$get.id;
  }, [options]);
  const handleSelectionChange = React8.useCallback((event, newValues) => {
    if (multiple) {
  ...
  const handleHighlightChange = React8.useCallback((event, newValue) => {
    onHighlightChange == null || onHighlightChange(event, newValue != null ? newValue : null);
  }, [onHighlightChange]);
  const handleStateChange = React8.useCallback((event, field, fieldValue) => {
    if (field === "open") {
  ...
  const getItemDomElement = React8.useCallback((itemId) => {
  ...
  const useListParameters = {
    ...
    reducerActionContext: React8.useMemo(() => ({
      multiple
    }), [multiple]),
    items: optionValues,
  ...
  const isInitiallyOpen = React8.useRef(open);
  useEnhancedEffect_default(() => {
  ...
  const getOptionMetadata = React8.useCallback((optionValue) => getOptionByValue(optionValue), [getOptionByValue]);
  ...
  const contextValue = React8.useMemo(() => _extends({}, listContextValue, compoundComponentContextValue), [listContextValue, compoundComponentContextValue]);
  let selectValue;
  if (props.multiple) {
...
function SelectProvider(props) {
  ...
  const listContextValue = React9.useMemo(() => ({
    dispatch,
    getItemState,
    getItemIndex
...
function useSlider(parameters) {
  ...
  const touchId = React10.useRef(void 0);
  ...
  const previousIndex = React10.useRef(void 0);
...
function useTabs(parameters) {
  ...
  const onSelected = React13.useCallback((event, newValue) => {
    setValue(newValue);
  ...
  const tabIdLookup = React13.useRef(() => void 0);
  const getTabPanelId = React13.useCallback((tabValue) => {
    var _tabPanels$get;
    return (_tabPanels$get = tabPanels.get(tabValue)) == null ? void 0 : _tabPanels$get.id;
  }, [tabPanels]);
  const getTabId = React13.useCallback((tabPanelId) => {
    return tabIdLookup.current(tabPanelId);
  }, []);
  const registerTabIdLookup = React13.useCallback((lookupFunction) => {
...
function TabsProvider(props) {
  ...
  const tabsContextValue = React15.useMemo(() => ({
    direction,
    getTabId,
    getTabPanelId,
    onSelected,
  ...
  return (0, import_jsx_runtime5.jsx)(CompoundComponentContext.Provider, {
    value: compoundComponentContextValue,
    children: (0, import_jsx_runtime5.jsx)(TabsContext.Provider, {
...
var Tabs = React16.forwardRef(function Tabs2(props, forwardedRef) {
  var _slots$root;
  const {
    children,
    orientation = "horizontal",
  ...
  return (0, import_jsx_runtime6.jsx)(TabsRoot, _extends({}, tabsRootProps, {
    children: (0, import_jsx_runtime6.jsx)(TabsProvider, {
      value: contextValue,
      children
    })
  }));
...
function useTabsList(parameters) {
  ...
  const tabIdLookup = React18.useCallback((tabValue) => {
    var _subitems$get;
    return (_subitems$get = subitems.get(tabValue)) == null ? void 0 : _subitems$get.id;
  }, [subitems]);
  registerTabIdLookup(tabIdLookup);
  const subitemKeys = React18.useMemo(() => Array.from(subitems.keys()), [subitems]);
  const getTabElement = React18.useCallback((tabValue) => {
  ...
  const handleChange = React18.useCallback((event, newValue) => {
    var _newValue$;
    onSelected(event, (_newValue$ = newValue[0]) != null ? _newValue$ : null);
  }, [onSelected]);
  const controlledProps = React18.useMemo(() => {
    if (value === void 0) {
      return {};
    }
    return value != null ? {
  ...
  const isItemDisabled = React18.useCallback((item) => {
  ...
  const {
    ...
    reducerActionContext: React18.useMemo(() => ({
  ...
  const contextValue = React18.useMemo(() => _extends({}, compoundComponentContextValue, listContextValue), [compoundComponentContextValue, listContextValue]);
  return {
    contextValue,
    dispatch,
    getRootProps,
...
function TabsListProvider(props) {
  ...
  const listContextValue = React19.useMemo(() => ({
    dispatch,
    getItemState,
    getItemIndex
  }), [dispatch, getItemIndex, getItemState]);
...
function useTab(parameters) {
  ...
  const tabMetadata = React20.useMemo(() => ({
    disabled,
    ref: tabRef,
    id
  }), [disabled, tabRef, id]);
...
var TextareaAutosize = React21.forwardRef(function TextareaAutosize2(props, forwardedRef) {
  ...
  return (0, import_jsx_runtime9.jsxs)(React21.Fragment, {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-VD45BBKO.js.map
